...
isnumber() { test "$*" && printf '%f' "$*" >/dev/null 2>/dev/null; }

# Checks if arg1 is an integer.  Will fail if
isinteger() { test "$*" && printf '%i' "$*" >/dev/null 2>/dev/null; }
...
increment_version() {
   ...
   # Get versionString string from file
   #versionString=`grep -o '[0-9]+\.?[0-9]*\.?[0-9]*}' trunk/manifest.json`
   versionString=`grep -o '[0-9]+(\.[0-9]+)*' trunk/manifest.json`
   fi
...
#sed -i "s/xxxxxxx/$version/g" trunk/manifest.json
#cd ..